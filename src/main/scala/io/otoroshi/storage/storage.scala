package io.otoroshi.storage

import akka.util.ByteString
import io.otoroshi.env.Env
import io.otoroshi.events._
import io.otoroshi.gateway.RequestsDataStore
...
trait DataStores {
  def before(configuration: Configuration, lifecycle: ApplicationLifecycle): Future[Unit]
  def after(configuration: Configuration, lifecycle: ApplicationLifecycle): Future[Unit]
...
trait RedisLikeStore[T] extends BasicStore[T] {
  ...
  def findAllById(ids: Seq[String])(implicit ec: ExecutionContext, env: Env): Future[Seq[T]] = ids match {
    ...
    case keys if findAllCache.get() != null => {
      val _t1 = env.metrics.timer("findAllById").time()
      val servs = findAllCache.get().filter(s => keys.contains(extractId(s)))
      _t1.close()
      FastFuture.successful(servs)
    }
    case keys if findAllCache.get() == null =>
      val _t1 = env.metrics.timer("findAllById").time()
      redisLike.mget(keys.map(keyStr): _*).map { values: Seq[Option[ByteString]] =>
        val v = values.flatMap { opt =>
