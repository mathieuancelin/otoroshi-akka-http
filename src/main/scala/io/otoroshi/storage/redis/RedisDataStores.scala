...
class RedisDataStores(configuration: Configuration, lifecycle: ApplicationLifecycle) extends DataStores {
  ...
  lazy val redisActorSystem =
    ActorSystem("redis", configuration.getConfig("app.redis.akka").getOrElse(Configuration.empty).underlying)
