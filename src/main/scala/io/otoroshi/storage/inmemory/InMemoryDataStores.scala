...
class InMemoryDataStores(env: Env, configuration: Configuration, lifecycle: ApplicationLifecycle) extends DataStores {
  ...
  //  ActorSystem("inmemory", configuration.getConfig("app.inmemory.akka").getOrElse(Configuration.empty).underlying)
  lazy val redis = new InMemoryRedis(env.system)
