...
class InMemoryServiceDescriptorDataStore(redisCli: RedisLike, maxQueueSize: Int)
  ...
  override def find(query: ServiceDescriptorQuery)(implicit ec: ExecutionContext,
    ...
    val _t1 = env.metrics.timer("inside_service_find").time()
    val _t2 = env.metrics.timer("inside_service_find_exists").time()
    query.exists().flatMap {
      case true => {
        _t2.close()
        logger.debug(s"Service descriptors for $query")
        val _t3 = env.metrics.timer("inside_service_find_getServices").time()
        query
          .getServices()
          .map { services =>
            _t3.close()
            val _t4 = env.metrics.timer("inside_service_find_filter").time()
            val filtered = services.filter { sr =>
        ...
        findAll().map { descriptors =>
          val validDescriptors = descriptors.filter { sr =>
            if (env.redirectToDev) { // TODO : prefer preprod
              ...
              RegexPool(sr.toDevHost).matches(query.toDevHost)
            } else {
              ...
              RegexPool(sr.toHost).matches(query.toHost)
            }
          }
      ...
      case _ =>
        _t1.close()
        // logger.debug(s"Found microservice in ${System.currentTimeMillis() - start} ms.")
    }
  }
