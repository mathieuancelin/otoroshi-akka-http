...
class InMemoryApiKeyDataStore(redisCli: RedisLike) extends ApiKeyDataStore with RedisLikeStore[ApiKey] {
  ...
  override def remainingQuotas(apiKey: ApiKey)(implicit ec: ExecutionContext, env: Env): Future[RemainingQuotas] =
    for {
      secCalls     <- redisCli.get(throttlingKey(apiKey.clientId)).map(_.map(_.utf8String.toLong).getOrElse(0L))
      dailyCalls   <- redisCli.get(dailyQuotaKey(apiKey.clientId)).map(_.map(_.utf8String.toLong).getOrElse(0L))
      monthlyCalls <- redisCli.get(monthlyQuotaKey(apiKey.clientId)).map(_.map(_.utf8String.toLong).getOrElse(0L))
    } yield
      RemainingQuotas(
  ...
  override def withinDailyQuota(apiKey: ApiKey)(implicit ec: ExecutionContext, env: Env): Future[Boolean] =
    redisCli.get(dailyQuotaKey(apiKey.clientId)).map(_.map(_.utf8String.toLong).getOrElse(0L) <= apiKey.dailyQuota)

  override def withinMonthlyQuota(apiKey: ApiKey)(implicit ec: ExecutionContext, env: Env): Future[Boolean] =
    redisCli.get(monthlyQuotaKey(apiKey.clientId)).map(_.map(_.utf8String.toLong).getOrElse(0L) <= apiKey.monthlyQuota)
      ...
      case Some(descriptor) => {
        ...
        redisCli.exists(key).flatMap {
          case true => {
            ...
            redisCli.smembers(key).map(_.map(_.utf8String)).flatMap { ids =>
  ...
  override def findAuthorizeKeyFor(clientId: String, serviceId: String)(implicit ec: ExecutionContext,
    ...
    findById(clientId).flatMap {
      case Some(apiKey) => apiKey.services.map(services => services.find(_.id == serviceId).map(_ => apiKey))
