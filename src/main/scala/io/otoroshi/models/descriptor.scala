...
import io.otoroshi.security.IdGenerator
...
case class ServiceDescriptorQuery(subdomain: String,
  ...
  def getServices()(implicit ec: ExecutionContext, env: Env): Future[Seq[ServiceDescriptor]] = {
    val key = this.asKey
    val _t1 = env.metrics.timer("inside_getServices_get").time()
    get().flatMap { ids =>
      _t1.close()
      val _t2 = env.metrics.timer("inside_getServices_findAllById").time()
      if (!servicesCache.containsKey(key)) {
        env.datastores.serviceDescriptorDataStore.findAllById(ids).andThen {
          case scala.util.Success(ex) =>
        ...
        FastFuture.successful(servicesCache.get(key)).andThen {
          case _ => _t2.close()
        }
      }
    }
...
case class Target(host: String, scheme: String = "https") {
  ...
  lazy val asAkkaHost: (Uri.Host, Int) = (scheme, host.split(":").toList) match {
    case (_, h :: p :: Nil)  => (Uri.Host(h), p.toInt)
    case ("http", h :: Nil)  => (Uri.Host(h), 80)
    case ("https", h :: Nil) => (Uri.Host(h), 443)
    case _                   => (Uri.Host(host), 80)
  }
}
...
case class ServiceDescriptor(
  ...
  def isUriPublic(uri: String): Boolean =
    !privatePatterns.exists(p => RegexPool.regex(p).matches(uri)) && publicPatterns.exists(
      p => RegexPool.regex(p).matches(uri)
    )
  def isUriExcludedFromSecuredCommunication(uri: String): Boolean =
    !secComExcludedPatterns.exists(p => RegexPool.regex(p).matches(uri))
