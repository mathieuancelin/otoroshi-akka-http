...
import io.otoroshi.security.Auth0Config
import io.otoroshi.utils.CleverCloudClient
import io.otoroshi.utils.CleverCloudClient.{CleverSettings, UserTokens}
...
trait GlobalConfigDataStore extends BasicStore[GlobalConfig] {
  ...
  def withinThrottlingQuota()(implicit ec: ExecutionContext, env: Env): Future[Boolean]
  def updateQuotas(config: GlobalConfig)(implicit ec: ExecutionContext, env: Env): Future[Unit]
